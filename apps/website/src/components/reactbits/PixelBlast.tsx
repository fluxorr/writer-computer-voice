...
const createTouchTexture = (): TouchTexture => {
  ...
  const drawPoint = (p: TouchPoint) => {
    ...
    intensity *= p.force;
    const r = Math.round(((p.vx + 1) / 2) * 255);
    const g = Math.round(((p.vy + 1) / 2) * 255);
...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    const needsReinit =
      ...
      antialias !== prevConfigRef.current?.antialias ||
      liquid !== prevConfigRef.current?.liquid ||
      noiseAmount !== prevConfigRef.current?.noiseAmount;
    ...
    if (needsReinit) {
      ...
      const animate = () => {
        ...
        if (liquidEffect) {
          const liqEffect = liquidEffect as any;
