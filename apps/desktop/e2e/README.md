...
End-to-end tests for the Speakdown desktop app (a fork of Writer by joelbqz, maintained at `fluxorr/speakdown`), driven via WebdriverIO and the
...
> The e2e build uses an isolated bundle identifier (`dev.fluxorr.speakdown.e2e`)
> so it does NOT collide with `tauri-plugin-single-instance` from a Speakdown
> dev or release instance running in another worktree. You can leave your
> normal Speakdown running.
...
1. `pnpm run build:app` produces `Speakdown.app` with the `e2e` Cargo feature,
...
2. `wdio.conf.js` (`onPrepare`) spawns the `tauri-webdriver` intermediary CLI
   on port 4444.
3. WebdriverIO connects to 4444; the intermediary launches `Speakdown.app` and
...
- The e2e build invokes
  `cargo tauri build --features e2e --bundles app --config '{"identifier":"dev.fluxorr.speakdown.e2e","bundle":{"createUpdaterArtifacts":false}}'`.
  The overrides:
  ...
  - `identifier: "dev.fluxorr.speakdown.e2e"` gives the e2e build its own
    `tauri-plugin-single-instance` namespace and its own app data dir
    (`~/Library/Application Support/dev.fluxorr.speakdown.e2e/`). Without this,
    a Speakdown dev/release instance running in another worktree would intercept
